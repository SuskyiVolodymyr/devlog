...
Agent routes stream tokens over SSE (`lib/sse.ts` wraps the loop's `onToken` callback). The UI (`AIPanel`) parses the stream, batches renders per animation frame, and shows text with a typewriter animation. Agents that feed UI components end their output with a sentinel-separated JSON block (`---` for prioritize, `[FLAGGED_JSON]` for backlog review) carrying exact task IDs — navigation never parses prose. `improve-task` is the exception: no tools, returns validated JSON, no streaming (it fills form fields).
